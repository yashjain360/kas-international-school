  ...
  return (
    <ErpLayout requiredRole="admin">
      <div className="space-y-6">
        ...
        {/* Faculty Grid */}
        {loading ? (
          <div className="text-center py-16 text-slate-500 text-sm">Loading Faculty Records...</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((fac) => (
              <div
                key={fac.id}
                className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xs"
              >
