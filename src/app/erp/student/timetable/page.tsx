    ...
    <ErpLayout requiredRole="student">
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Class Weekly Timetable Schedule</h1>
            <p className="text-xs text-slate-400">
              {user?.grade || 'Grade 10'}-{user?.section || 'A'} • Daily lecture periods, laboratory slots, and room locations.
            </p>
          </div>
        </div>
        ...
        {/* Timetable Grid */}
        {loading ? (
          <div className="text-center py-16 text-slate-500 text-sm">Loading Class Routine...</div>
        ) : (
          <div className="space-y-6">
            {days.map((day) => {
                ...
                <div key={day} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                    <h3 className="font-bold text-sm text-amber-400 flex items-center">
                      ...
                      {day} Schedule ({user?.grade || 'Grade 10'}-{user?.section || 'A'})
                    </h3>
